...
public class activatedFlow {
	
	DBCon db;
	
	public activatedFlow() throws SQLException{
		db = new DBCon();
	}
	
	public String ActivatedFirstFlow(msUserState user, stagingInput input){
		try {
			String message ="";
			...
			if(ri.checkPartnerAlias(user.getUsers())){
				message = "{message:\"berikut adalah list produk yang tidak aktif: ";
				for (String productName : pr.getProductsInactivate(ri.getPartnerId(user.getUsers()))) {
				message += "\n"+productName;
			} 
		...
		return "{message:\"Gagal Aktivasi product\" }";
	}
	
	
	public String ActiveProductFlow(msUserState user, stagingInput input){
		String message = "";
		try{
			ProductRegistrationInterface  pr = new ProductRegistration();
			...
			if(pr.getProductByNameIncativate(input.getMessage(),userID)!=null){
				StagingInputMethod sim = new StagingInputMethod();
				//masukan ke staging input
				sim.addStaging_input(input);
				message = "{message:\"Masukan nama produk yang ingin di aktifkan \"}";
	...
	public String ActivateGetConfirmationFlow(msUserState user, stagingInput input) throws JSONException{
		...
		try {
			...
			String productDetail = sim.selectByState("activate", 1, user.getUsers());
			
			
			RegistrationInterface regis = new RegistrationSQLServer();
			int userID = regis.getPartnerId(input.getUsername());
			msProduct product = pr.getProductByNameIncativate(productDetail, userID);
			msProduct m = new msProduct();
			
			m.setProduct_id(product.getProduct_id());
			//aktivasi produk
			ProductRegistrationInterface pi = new ProductRegistration();
			pi.activateProduct(m);
			...
			System.out.println("email berhasil terkirim ke " + user.getUsers());
			...
			user.setStateNum(0);
			userState.updateUser(user);
			return "{message:\"Produk Telah Berhasil di aktifasi! \" }";
		} catch (SQLException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
	...
	public String cancelActiveFlow(msUserState user ,stagingInput input){
		JSONObject info = new JSONObject();
		try {
